package com.google.sites.medcare.Hospital.HospitalMap;
...
import com.google.sites.medcare.Hospital.HospitalMap.adapter.LocationRecyclerViewAdapter;
import com.google.sites.medcare.Hospital.HospitalMap.model.IndividualLocation;
import com.google.sites.medcare.Hospital.HospitalMap.util.LinearLayoutManagerWithSmoothScroller;
...
public class MapActivity extends AppCompatActivity implements
  ...
  //private static final LatLngBounds LOCKED_MAP_CAMERA_BOUNDS = new LatLngBounds.Builder().include(new LatLng(40.87096725853152, -74.08277394720501)).include(new LatLng(40.67035340371385, -73.87063900287112)).build();
    ...
    if (!features.isEmpty()) {
      ...
      for (int i = 0; i < featureList.size(); i++) {
        ...
        if (featureList.get(i).getStringProperty("name").equals(name)) {
          ...
          if (selectedFeaturePoint.latitude() != wayLatitude) {
  ...
  private void getInformationFromDirectionsApi(Point destinationPoint, final boolean fromMarkerClick, @Nullable final Integer listIndex) {
    // Set up origin and destination coordinates for the call to the Mapbox Directions API
    Point mockCurrentLocation = Point.fromLngLat(wayLongitude, wayLatitude);
    ...
    NavigationRoute.builder(this)
            ...
            .getRoute(new Callback<DirectionsResponse>() {
              ...
              public void onResponse(Call<DirectionsResponse> call, Response<DirectionsResponse> response) {
                // Check that the response isn't null and that the response has a route
                if (response.body() == null) {
                  Log.e("MapTesting", "No routes found, make sure you set the right user and access token.");
                } else if (response.body().routes().size() < 1) {
                  Log.e("MapTesting", "No routes found");
                } else {
                  if (fromMarkerClick) {
                    // Retrieve and draw the navigation route on the map
                    currentRoute = response.body().routes().get(0);
  ...
  private void addMockDeviceLocationMarkerToMap() {
    ...
    if (style != null) {
      ...
      style.addSource(new GeoJsonSource("mock-device-location-source-id", Feature.fromGeometry(
        Point.fromLngLat(wayLongitude, wayLatitude))));
    ...
    try {
      ...
      rootRef.addValueEventListener(new ValueEventListener() {
        ...
        public void onDataChange(DataSnapshot dataSnapshot) {
          ...
          Log.d("json","jsonobject: " + object);
          Log.d("json","json: " + json[0]);

          featureCollection = FeatureCollection.fromJson(json[0]);
          Log.d("json","feature1: " + featureCollection);
        }
      ...
      /*featureCollection = FeatureCollection.fromJson(json[0]);
      Log.d("json","feature2: " + featureCollection);*/
      featureCollection = FeatureCollection.fromJson(loadGeoJsonFromAsset("list_of_locations.geojson"));
      Log.d("json","feature: " + featureCollection);
      //Log.d("json","json: " + featureCollection);
    } catch (Exception exception) {
